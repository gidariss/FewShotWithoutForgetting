    ...
    for _ in range(100):
        ...
        if dloader.phase != "train":
            ...
            assert (
                len(
                    set.intersection(
                        set(all_img_ids_of_base_categories), set(novel_img_ids)
                    )
                )
                == 0
            )
            assert len(
                set.intersection(
                    set(all_img_ids_of_novel_categories), set(novel_img_ids)
                )
            ) == len(novel_img_ids)
        ...
        assert len(set.intersection(set(Kall[:nKbase]), set(Kall[nKbase:]))) == 0
