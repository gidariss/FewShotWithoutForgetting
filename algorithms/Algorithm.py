...
class Algorithm:
    def __init__(self, opt):
        ...
        # )
        self.keep_best_model_metric_name = None # @TODO
    ...
    def load_pretrained(self, network, pretrained_path):
        ...
        self.logger.info(
            "==> Load pretrained parameters from file %s:" % pretrained_path
        )
    ...
    def init_optimizer(self, net, optim_opts, key):
        ...
        if optim_type == "adam":
            optimizer = torch.optim.Adam(
                parameters, lr=learning_rate, betas=optim_opts["beta"]
            )
        elif optim_type == "sgd":
            optimizer = torch.optim.SGD(
                ...
                nesterov=optim_opts["nesterov"]
                if ("nesterov" in optim_opts)
                else False,
    ...
    def init_all_criterions(self):
        ...
        for key, val in criterions_defs.items():
            ...
            self.logger.info(
                "Initialize criterion[%s]: %s with options: %s"
    ...
    def find_most_recent_epoch(self, key, suffix):
        ...
        self.logger.info(
            "Load checkpoint of most recent epoch %s" % str(most_recent_epoch)
        )
    ...
    def load_network(self, net_key, epoch, suffix=""):
        assert net_key in self.networks
        filename = self._get_net_checkpoint_filename(net_key, epoch) + suffix
        self.logger.info("Loading {0} for network {1}".format(filename, net_key))
    ...
    def evaluate(self, dloader):
        ...
        self.logger.info(
            "==> Dataset: %s [%d batches]" % (dloader.dataset.name, len(dloader))
        )
        for key, network in self.networks.items():
            network.eval()
    ...
    def keep_record_of_best_model(self, eval_stats, current_epoch):
        if self.keep_best_model_metric_name is not None:
            ...
            if metric_name not in eval_stats:
                raise ValueError(
                    "The provided metric {0} for keeping the best "
                    "model is not computed by the evaluation routine.".format(
                        metric_name
                    )
                )
